...
def process_voice_message(message,file_info):
    ...
    ogg2wav(os.path.join('temp', src_filename),os.path.join('temp', dest_filename))
    bot.reply_to(message, '3')
    # bot.reply_to(message, '3')
    # bot.reply_to(os.path.dirname(__file__))
    # bot.reply_to(message,os.path.join(os.path.dirname(__file__),"temp",dest_filename))
    # bot.reply_to(message,Feature(os.path.join(os.path.dirname(__file__),"temp",dest_filename)).extract_feature())
    # Use the model to predict the emotion
    from sklearn.preprocessing import StandardScaler
    sc = StandardScaler()
    # X_train = sc.fit_transform(X_train)
    # X_test = sc.transform(X_test)
    # emotion = rec.predict([sc.transform(Feature(os.path.join(os.path.dirname(__file__),"temp",dest_filename)).extract_feature())])
    emotion = predict(os.path.join(os.path.dirname(__file__),"temp",dest_filename))
    # print(emotion)
    # return
