...
# from modules.ClassifiersManagement.OneOutputMLPClassifier import get_classifier,predict,get_classifier_through_randomized_search_cv
...
# rec = test_9_emotions()
rec = get_classifier()
# rec = get_classifier_through_randomized_search_cv()
...
def handle_audio_message(message):
    ...
    ogg2wav(received_file_path,destination_file_path)
    reduce_noise(destination_file_path,destination_after_reduce_noise_file_path)
    scale_amplitude(destination_file_path,destination_file_path,0.3)
    scale_amplitude(destination_after_reduce_noise_file_path,destination_after_reduce_noise_file_path,0.3)


    # sig1 = print_signal_array(received_file_path)
    # sig2 = print_signal_array(destination_file_path)
    # print('abd',sig1)
    # print('abd',sig2)
    # print('abd',sum(sig1!=sig2))
    # process_voice_message(message,received_file_path)
    process_voice_message(message, destination_file_path)
    process_voice_message(message, destination_after_reduce_noise_file_path)
...
def handle_wav_file(message):
    ...
    received_file_path = os.path.join(os.path.dirname(
        __file__), 'temp', f"{message.document.file_name}")
    ...
    reduce_noise(received_file_path,destination_after_reduce_noise_file_path)
    scale_amplitude(received_file_path,received_file_path,0.3)
    scale_amplitude(destination_after_reduce_noise_file_path,destination_after_reduce_noise_file_path,0.3)
