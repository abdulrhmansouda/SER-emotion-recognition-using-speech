...
def get_classifier():
    try:
        ...
        clf = MLPClassifier(solver='adam', max_iter=500, learning_rate='adaptive', hidden_layer_sizes=(
            256, 256), batch_size='auto', alpha=0.005, activation='tanh')
...
if __name__ == '__main__':
    ...
    helper.confusion_matrix(y_test=y_test, y_prediction=y_prediction, classifier_name='RandomizeSearch_MLPClassifier',title=f"Accuracy: {clf.score(X_test, y_test)} - RandomizeSearch_MLPClassifier\n{helper.get_special_name(folder_name='',prefix='')}\n{clf.best_params_}")
