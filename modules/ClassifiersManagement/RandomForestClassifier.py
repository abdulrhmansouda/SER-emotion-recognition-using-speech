...
def confusion_matrix(y_test, y_prediction, title='', normalize=None):
    ...
    # plt.close()
    plt.draw()
    fig1.savefig(helper.get_special_name(folder_name='statistics\\RandomForestClassifier', prefix=str(
        time.time())+'_RandomForestClassifier', extension='.png'), bbox_inches='tight')
    # plt.savefig(helper.get_special_name('pickled', str(time.time())+'_RandomForestClassifier','.png'),dpi=300)
